...
import '../widgets/kelola_stok/tambah_produk_produksi_dialog.dart';

class OwnerTambahkanProduksiStokPage extends StatefulWidget {
  const OwnerTambahkanProduksiStokPage({
    super.key,
  ...
  final ProduksiStokEntry? existingEntry;

  @override
  State<OwnerTambahkanProduksiStokPage> createState() =>
      _OwnerTambahkanProduksiStokPageState();
}

class _OwnerTambahkanProduksiStokPageState
    extends State<OwnerTambahkanProduksiStokPage> {
  ...
  void initState() {
    ...
    if (existing != null) {
      ...
      _kode = widget.provider.generateKodeProduksi();
        ...
        child: Column(
          ...
          children: [
            ...
            _BottomButtons(
              ...
              onDraft: () => _save(ProduksiStokStatus.draft),
              onPostNow: () => _save(ProduksiStokStatus.posted),
            ),
          ],
        ),
      ),
  ...
  Future<void> _onTambahProduk() async {
    ...
    final items = await TambahProdukProduksiPage.push(
      context,
  ...
  Future<void> _save(ProduksiStokStatus status) async {
    if (_produk.isEmpty) {
    ...
    if (widget.existingEntry != null) {
      ...
      widget.provider.updateProduksiStok(entry);
      navigator.pop();
      return;
    }

    final error = await widget.provider.addProduksiStok(entry);
    if (!mounted) return;
    if (error != null) {
...
class _CreatedByRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          Text('Created By', style: AppTypography.textTheme.bodyMedium),
          const Spacer(),
          Text(createdBy, style: AppTypography.textTheme.bodyMedium),
        ],
      ),
    );
  }
...
class _ProdukSection extends StatelessWidget {
  ...
  final ValueChanged<ProduksiStokProdukItem> onDelete;
  final void Function(ProduksiStokProdukItem, int) onQuantityChanged;

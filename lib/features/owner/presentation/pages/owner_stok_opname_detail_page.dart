...
class OwnerStokOpnameDetailPage extends StatelessWidget {
  const OwnerStokOpnameDetailPage({super.key, required this.entry});
            ...
            Expanded(
              child: ListView(
                children: [
                  ...
                  const Divider(height: 1),
                  _buildSection('Catatan', entry.catatan ?? '',
  ...
  Widget _buildDocumentHeader() {
    final (bgColor, icon, statusLabel, statusColor) = switch (entry.status) {
      StokOpnameStatus.posted => (
          AppColors.tertiary,
          Icons.check_rounded,
          'Posted',
          AppColors.tertiary,
        ),
      StokOpnameStatus.draft => (
          Colors.orange,
          Icons.bookmark_rounded,
          'Draft',
          Colors.orange,
        ),
      StokOpnameStatus.cancelled => (
          AppColors.error,
          Icons.close_rounded,
          'Cancelled',
          AppColors.error,
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: AppSpacing.x2),
          _ReadOnlyBox(value: item.qtyAktual),
          const SizedBox(width: AppSpacing.x3),

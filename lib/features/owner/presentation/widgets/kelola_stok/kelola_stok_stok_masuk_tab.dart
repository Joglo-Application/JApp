...
import '../../pages/owner_stok_masuk_detail_page.dart';
import '../../pages/owner_tambahkan_stok_masuk_page.dart';
import '../../providers/kelola_stok_provider.dart';
import 'stok_masuk_list_item.dart';
import 'tambah_stok_masuk_dialog.dart';

class KelolaStokStokMasukTab extends StatelessWidget {
  const KelolaStokStokMasukTab({super.key});
...
class _RightPanelState extends State<_RightPanel> {
  ...
  Widget _buildList(BuildContext context) {
    final list = context.select<KelolaStokProvider, List<StokMasukEntry>>(
      (p) => p.stokMasukList,
    );
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) => StokMasukListItem(
        entry: filtered[i],
  ...
  void _onTapItem(
    BuildContext ctx,
    StokMasukEntry entry,
    KelolaStokProvider provider,
  ) {
    if (entry.status == StokMasukStatus.draft) {
      Navigator.of(ctx).push(
        MaterialPageRoute<void>(
          fullscreenDialog: true,
          builder: (_) => OwnerTambahkanStokMasukPage(
            provider: provider,
      ...
      Navigator.of(ctx).push(
        MaterialPageRoute<void>(
          fullscreenDialog: true,
          builder: (_) => OwnerStokMasukDetailPage(entry: entry),
        ),
      );
    }
  }
  ...
  Future<void> _navigateToTambah(BuildContext context) async {
    ...
    final result = await TambahStokMasukDialog.show(context);

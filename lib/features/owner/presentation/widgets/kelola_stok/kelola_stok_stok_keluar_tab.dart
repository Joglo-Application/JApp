...
import '../../pages/owner_stok_keluar_detail_page.dart';
import '../../pages/owner_tambahkan_stok_keluar_page.dart';
import '../../providers/kelola_stok_provider.dart';
import 'tambah_stok_keluar_dialog.dart';

class KelolaStokStokKeluarTab extends StatelessWidget {
  const KelolaStokStokKeluarTab({super.key});
...
class _RightPanelState extends State<_RightPanel> {
  ...
  Widget _buildList(BuildContext context) {
    final list = context.select<KelolaStokProvider, List<StokKeluarEntry>>(
      (p) => p.stokKeluarList,
    );
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) => _StokKeluarListItem(
        entry: filtered[i],
  ...
  void _onTapItem(
    BuildContext ctx,
    StokKeluarEntry entry,
    KelolaStokProvider provider,
  ) {
    if (entry.status == StokKeluarStatus.draft) {
      Navigator.of(ctx).push(
        MaterialPageRoute<void>(
          fullscreenDialog: true,
          builder: (_) => OwnerTambahkanStokKeluarPage(
            provider: provider,
      ...
      Navigator.of(ctx).push(
        MaterialPageRoute<void>(
          fullscreenDialog: true,
          builder: (_) => OwnerStokKeluarDetailPage(entry: entry),
        ),
      );
    }
  }
  ...
  Future<void> _navigateToTambah(BuildContext context) async {
    ...
    final result = await TambahStokKeluarDialog.show(context);
...
class _StokKeluarListItem extends StatelessWidget {
  const _StokKeluarListItem({required this.entry, this.onTap});
...
class _StatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, color) = switch (status) {
      StokKeluarStatus.posted => ('Posted', AppColors.tertiary),
      StokKeluarStatus.draft => ('Draft', Colors.orange),
      StokKeluarStatus.cancelled => ('Cancelled', AppColors.error),
    };
